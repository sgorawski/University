...
public class DataParser {
    ...
    public static List<EdgeData> readEdgesFromFile(String path) throws IOException {
        ...
        for (String line: lines
            ...
            if (Pattern.compile("->").matcher(els[0]).find()) {
                ...
                edges.add(new EdgeData(vertices[0].trim(), vertices[1].trim(),
                ...
                edges.add(new EdgeData(vertices[0].trim(), vertices[1].trim(),
                        Double.parseDouble(els[1].trim().replace(",", "."))));
                edges.add(new EdgeData(vertices[1].trim(), vertices[0].trim(),
                        Double.parseDouble(els[1].trim().replace(",", "."))));
            }
        }
        return edges;
    ...
    public static void saveEdgesToFile(Iterable<Edge> edges, String path) throws IOException {
        ...
        try (PrintWriter out = new PrintWriter(path)) {
            for (Edge edge: edges
                ...
                out.println(String.format("%s -> %s:%f",
                        edge.getStartVertex().getName(), edge.getFinishVertex().getName(), edge.getCost()));
            }
        }
    }
    ...
    public static Collection<LineData> readLinesFromFile(String path) throws IOException {
        List<String> lines = Files.readAllLines(Paths.get(path), Charset.defaultCharset());
        return lines.stream().map(DataParser::toLine).filter(Objects::nonNull).collect(Collectors.toList());
    }
    ...
    private static LineData toLine(String ln) {
        ...
        return new LineData(params[0].trim(), params[1].trim(), Integer.parseInt(params[2].trim()), verticesNames);
    }
}
