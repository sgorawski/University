...
class Picross:
    ...
    def opt_dist(self, digits, ns):
        key = tuple(digits), tuple(ns)
        ...
        n = ns[0]
        last = len(digits) - sum(ns) - len(ns) + 1
        flips_before_window = 0
        num_1s_in_window = sum(digits[:n])

        # print(digits, ns, last)
        to_flip_just_after = len(digits) > n and digits[n]
        least_flips = (n - num_1s_in_window) + to_flip_just_after + self.opt_dist(digits[n+1:], ns[1:])

        if last > 0:
            for i in range(last):
                num_1s_in_window -= digits[i]
                flips_before_window += digits[i]
                num_1s_in_window += digits[i + n]
                to_flip_just_after = len(digits) > i + n + 1 and digits[i + n + 1]

                least_flips = min(
                    (n - num_1s_in_window) + flips_before_window + to_flip_just_after + self.opt_dist(digits[i+n+2:], ns[1:]),
                    least_flips
                )
